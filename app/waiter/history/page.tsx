...
export default async function WaiterHistoryPage({ searchParams }: WaiterHistoryPageProps) {
    const user = await requireServerSessionUser(["waiter"]);
    ...
    return (
        <AppShell user={user} active="waiter.history">
            <div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    <h1 className="font-display text-2xl font-bold text-moka-ink">Riwayat Pesanan Waiter</h1>
                    <p className="text-sm text-moka-muted">Pesanan yang kamu buat hari ini dan menunggu diproses kasir.</p>
                </div>
                <Link href="/waiter" className="moka-btn-secondary">
                    Kembali ke Order
                </Link>
            </div>
            ...
            <Pagination basePath="/waiter/history" currentPage={currentPage} totalPages={totalPages} totalItems={totalRows} perPage={perPage} />
        </AppShell>
    );
}
