    ...
    return (
        <AppShell user={user} active="waiter.history">
            <div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    <h1 className="font-display text-2xl font-bold text-moka-ink">Detail Pesanan</h1>
                    <p className="text-sm text-moka-muted">
                        {displayInvoice(order)} - {formatDateTime(order.orderedAt)}
                    </p>
                </div>
                <Link href="/waiter/history" className="moka-btn-secondary">
                    Kembali ke Riwayat
                </Link>
            </div>
                ...
                <div className="soft-card overflow-hidden p-0">
                    ...
                    <div className="divide-y divide-moka-line">
                        {order.items.length === 0 ? (
                            <p className="px-5 py-8 text-center text-sm text-moka-muted">Item pesanan kosong.</p>
                        ) : (
                    ...
                    <dl className="space-y-2 text-sm text-moka-muted">
                        ...
                        <div className="flex items-center justify-between">
                            ...
                            <dd>
                                <Badge
                                    variant={
                                        order.status === OrderStatus.PAID ? "success" : order.status === OrderStatus.WAITING ? "warning" : "danger"
                                    }
                                >
                                    {order.status}
                                </Badge>
                        ...
                        <div className="flex items-center justify-between">
                            <dt>Metode Bayar</dt>
                            <dd className="font-semibold text-moka-ink">{order.status === OrderStatus.WAITING ? "-" : order.paymentMethod}</dd>
                        </div>
