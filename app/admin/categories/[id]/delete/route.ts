...
export async function POST(request: NextRequest, context: Context): Promise<NextResponse> {
    ...
    try {
        ...
        return withFlash(response, { type: "error", message: "Kategori tidak bisa dihapus karena masih dipakai produk." });
    }

    const response = NextResponse.redirect(new URL("/admin/categories", request.url), { status: 303 });
    return withFlash(response, { type: "success", message: "Kategori berhasil dihapus." });
}
